...
from LabExT.Movement.Calibration import AxesRotation, Axis, Direction
import pytest
import unittest
import numpy as np
...
from LabExT.Movement.Transformations import CoordinatePairing, KabschRotation, SinglePointFixation, make_3d_coordinate
...
class FooTest(unittest.TestCase):
    ...
    def test_foo(self):
        self.kabsch_1 = KabschRotation()
        self.kabsch_2 = KabschRotation()

        self.rotation_left = AxesRotation()
        self.rotation_right = AxesRotation()

        self.rotation_left.update(Axis.X, Axis.X, Direction.NEGATIVE)
        self.rotation_left.update(Axis.Y, Axis.Y, Direction.NEGATIVE)
        self.rotation_left.update(Axis.Z, Axis.Z, Direction.NEGATIVE)

        self.rotation_right.update(Axis.Z, Axis.Z, Direction.NEGATIVE)
        ...
        self.kabsch_2.update(
            CoordinatePairing(
                self.calibration,
                stage_coordinate_1,
                device_1,
                self.rotation_left.chip_to_stage(device_1._out_position)))
        ...
        self.kabsch_1.update(
            CoordinatePairing(
                ...
                stage_coordinate_2,
                device_2,
                self.rotation_left.chip_to_stage(device_2._out_position)))
        ...
        self.kabsch_1.update(
            CoordinatePairing(
                ...
                stage_coordinate_3,
                device_3,
                self.rotation_left.chip_to_stage(device_3._out_position)))
