...
from LabExT.Movement.config import DevicePort, Direction, Orientation, State, Axis
from LabExT.Movement.Transformations import ChipCoordinate, CoordinatePairing, SinglePointOffset, AxesRotation, KabschRotation, StageCoordinate
...
class Calibration:
    ...
    AVAILABLE_SYSTEMS = [ChipCoordinate, StageCoordinate]
    ...
    def update_single_point_offset(self, pairing: Type[CoordinatePairing]) -> None:
        """
    ...
    def coordinate_system(self):
        """
        Returns the current coordinate system in which the stage operates.
        """
    ...
    def coordinate_system(self, coordinate_system):
        ...
        if coordinate_system not in self.AVAILABLE_SYSTEMS:
            raise ValueError(f"The requested coordinate system {coordinate_system} is not supported.")
    ...
    @contextmanager
    def in_chip_coordinates(self):
        """
        Sets the current coordinate system for chip coordinates,
        such that all methods in the context operate in this system.
        """
    ...
    @contextmanager
    def in_stage_coordinates(self):
        """
        Sets the current coordinate system for stage coordinates,
        such that all methods in the context operate in this system.
        """
    ...
    def position(self) -> Union[Type[StageCoordinate], Type[ChipCoordinate]]:
        ...
        """
        stage_position = StageCoordinate.from_list(self.stage.position)
        ...
        if self.coordinate_system == StageCoordinate:
            ...
            if self.state == State.FULLY_CALIBRATED:
                ...
                raise CalibrationError("Insufficient calibration state to return the position in chip coordinates.")
        else:
            RuntimeError(f"Unsupported coordinate system {self.coordinate_system} to return the stage position")


    @assert_minimum_state_for_coordinate_system(
        stage_coordinate_system=State.CONNECTED,
        chip_coordinate_system=State.COORDINATE_SYSTEM_FIXED)
    def move_relative(self, offset: Union[Type[StageCoordinate], Type[ChipCoordinate]], wait_for_stopping: bool = True) -> None:
        """
        ...
        if not isinstance(offset, self.coordinate_system):
            raise TypeError(f"Given offset is in {type(offset)}. Need offset in {self.coordinate_system} to move the stage relative in this system.")
        ...
        if self.coordinate_system == StageCoordinate:
            ...
            RuntimeError(f"Unsupported coordinate system {self.coordinate_system} to move the stage relatively.")

        self.stage.move_relative(*stage_offset.to_list(), wait_for_stopping=wait_for_stopping)
    ...
    @assert_minimum_state_for_coordinate_system(
        stage_coordinate_system=State.CONNECTED,
        chip_coordinate_system=State.SINGLE_POINT_FIXED)
    def move_absolute(self, coordinate: Union[Type[StageCoordinate], Type[ChipCoordinate]], wait_for_stopping: bool = True) -> None:
        """
        ...
        if not isinstance(coordinate, self.coordinate_system):
            raise TypeError(f"Given coordinate is in {type(coordinate)}. Need coordinate in {self.coordinate_system} to move the stage absolute in this system.")
        ...
        if self.coordinate_system == StageCoordinate:
            ...
            if self.state == State.FULLY_CALIBRATED:
                stage_coordinate = self._kabsch_rotation.chip_to_stage(coordinate)
            elif self.state == State.SINGLE_POINT_FIXED:
                stage_coordinate = self._single_point_offset.chip_to_stage(coordinate)
            else:
                raise CalibrationError("Insufficient calibration state to move the stage absolutely.")
        else:
            RuntimeError(f"Unsupported coordinate system {self.coordinate_system} to move the stage absolutely.")

        self.stage.move_absolute(*stage_coordinate.to_list(), wait_for_stopping=wait_for_stopping)
